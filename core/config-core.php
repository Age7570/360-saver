...
";
$ga = str_replace("%id", $config["ga"], $code);
$config["ga"] = $ga;
$ads = array("ad728", "ad468", "ad300");
function get_ad( $ad){
	if(ADS==TRUE){
		GLOBAL $config;
		GLOBAL $ads;
		if(in_array($ad, $ads)){
			switch($ad){
				case $ads[0]:
					echo block_it($config[$ads[0]]);
				break;
		
				case $ads[1]:
					echo block_it($config[$ads[1]]);
				break;
		
				case $ads[2]:
					echo block_it($config[$ads[2]]);
				break;
			}
		}
	}
}
function block_it($ad) {
	return '<div class="col-md-12 mx-auto mb-5">'.$ad.'</div>';
}
